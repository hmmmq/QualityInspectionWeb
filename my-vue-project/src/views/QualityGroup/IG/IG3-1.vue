<template>
    ...
    <div v-for="(item, index) in processes" :key="index">
        <PGC9 :formArray="item" :head="'历史预验收记录'" :isShowButton="false" :isReadOnly2="true" />
    </div>
</template>
    ...
    methods: {
        ...
        transformData(data, type) {
            ...
            return data.map(item => {
                return Object.keys(item).map(key => {
                    ...
                    if (key === 'mainStructureStatus' || key === 'decorationStatus' || key === 'roofingStatus' || key === 'plumbingInstallationStatus' || key === 'sanitaryFixturesStatus' || key === 'electricalWiringStatus' || key === 'distributionBoxStatus' || key === 'ventilationDuctStatus' || key === 'airConditioningStatus' || key === 'overallQualityStatus' || key === 'documentationCompletenessStatus' || key === 'preAcceptanceConclusionStatus') {
                        if (value == 0) {
                            value = '待定';
                        } else if (value == 1) {
                            value = '已通过';
        ...
        getFieldType(key) {
            const typeMapping = {
                ...
                startDate: "date",
                completionDate: "date",
                mainStructureStatus: "text",
                mainStructureReason: "text",
                decorationStatus: "text",
                decorationReason: "text",
                roofingStatus: "text",
                roofingReason: "text",
                plumbingInstallationStatus: "text",
                plumbingInstallationReason: "text",
                sanitaryFixturesStatus: "text",
                sanitaryFixturesReason: "text",
                electricalWiringStatus: "text",
                electricalWiringReason: "text",
                distributionBoxStatus: "text",
                distributionBoxReason: "text",
                ventilationDuctStatus: "text",
                ventilationDuctReason: "text",
                airConditioningStatus: "text",
                airConditioningReason: "text",
                overallQualityStatus: "text",
                overallQualityReason: "text",
                documentationCompletenessStatus: "text",
                documentationCompletenessReason: "text",
                preAcceptanceConclusionStatus: "text",

<template>
    ...
    <div v-for="(item, index) in processes" :key="index">
        <PGC9 :formArray="item" :head="'历史正式联合验收记录'" :isShowButton="false" :isReadOnly2="true" />
    </div>
</template>
    ...
    methods: {
        ...
        transformData(data, type) {
            ...
            return data.map(item => {
                return Object.keys(item).map(key => {
                    ...
                    if (key === 'subdivisionAcceptanceStatus' || key === 'qualityControlDataStatus' || key === 'safetyFunctionCheckStatus' || key === 'appearanceQualityStatus') {
                        if (value == 0) {
                            value = '待定';
                        } else if (value == 1) {
                            value = '已通过';
        ...
        getFieldType(key) {
            const typeMapping = {
                ...
                subdivisionAcceptanceStatus: "text",
                qualityControlDataStatus: "text",
                safetyFunctionCheckStatus: "text",
                appearanceQualityStatus: "text",
